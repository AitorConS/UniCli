...
For the `jerboa` package source, the remote package index is cached at
`~/.jerboa/packages/index.json` after a successful fetch. If the remote index is
unavailable, `jerboa build`, `jerboa pkg search`, and `jerboa pkg get` fall back
to the cached index; if no cached index exists, package resolution uses
downloaded local packages that have `meta.json`.
